...
class Manage_App:
    ...
    def minus_btn_on_search(self):
        ...
        if loan_minus > total_loan:
            messagebox.showerror("Error","The Entered  amount is greater than customer's actual loan")
            self.welcome()
            self.textarea.insert(END,f"\n The entered amount {str(loan_minus)} is greater than\n actual loan which is {total_loan}")
            self.textarea.insert(END,"\n Status: \t\t\tFailed")
            self.minus_loan.set(0)
            self.search_loan_name.set("")
            return
        asker = askstring("Are You sure!!","This will minus the loan amount of user\nand if amount is equal to total loan , it will clear all loan records of that customer\n Type \"yes\" to continue the process")
        try:
            asker.lower()
        except AttributeError:
            messagebox.showerror("Error","Operation not done, Denied by user")
        ...
        if asker.lower() != "yes":
            ...
            if loan_minus == total_loan:
                ...
                self.welcome()
                self.textarea.insert(END,f"\n Done. Current loan of {prod_name} is {hello}")
                self.textarea.insert(END,"\n Status: \t\t\tSuccess")
                self.minus_loan.set(0)
                self.search_loan_name.set("")
