...
class Bill_App:
    def __init__(self,root):
        ...
        self.Path_to_save_custommer_data = "D:\\files\\important files\\saved bills\\"
        self.Path_to_save_loan_data = "D:\\files\\important files\\saved loans\\"
...
##########################################################        
        ...
        #Image1
        img12=Image.open("D:\\files\\important files\\image\\mypic.jpg")
        ...
        #Image2
        img13=Image.open("D:\\files\\important files\\image\\newpic13.jpg")
    ...
    def welcome(self):
        ...
        self.textarea.insert(END,"\t\tNew SereenaZarai Corporation")
    ...
    def save_bill(self):
        if self.bill_no.get() == "":
            ...
            if op > 0:
                ...
                try:
                    f1=open("D:\\files\\important files\\Seperate_bills\\" + str(self.bill_no.get())+".txt","w",encoding="utf-8")
  ...
  #
    ...
    def find_bill(self):
        found = "No"
        for i in os.listdir("D:\\files\\important files\\saved bills\\"):
            if i.split(".txt")[0] == self.search_bill.get():
                f1=open(f"D:\\files\\important files\\saved bills\\{i}","r",encoding="utf-8")
                self.textarea.delete(1.0,END)
                for d in f1:
    ...
    def bill_search_by_name(self,event=""):
        ...
        if prod_name != "":
            ...
            for i in os.listdir("D:\\files\\important files\\saved loans\\"):
                lol = i.split(".txt")[0]
                if prod_name in lol:
...
#Manage
    def manage(self):
        self.root.destroy()
        os.system("python D:\\files\\manage.py")
    ...
    def fill_customers_fields_if_not_exists(self):
        ...
        if self.check_customers_name_notempty > 0:
            if self.check_customers_name_notexist > 0:
                ...
                with open(self.Path_To_Customer_data,"r",encoding="utf-8") as f:
                    data = json.load(f)
                with open("D:\\files\\important files\\database\\current_id.txt","r",encoding="utf-8") as h:
                    lol = h.read()
                lol = str(int(lol) + 1)
                data["customers"][prod_name]["phone"] = phone
                data["customers"][prod_name]["address"] = address
                data["customers"][prod_name]["customer_id"] = lol
                os.remove("D:\\files\\important files\\database\\current_id.txt")
                with open("D:\\files\\important files\\database\\current_id.txt" , "w" , encoding="utf-8") as q:
    ...
    def print_changing_customers_data(self):
        ...
        if loan.lower() == "false": 
            ...
            data["customers"][prod_name]["purchase"][bill_no]["payment"] = "Credit"
    ...
    def adding_to_loan_database(self):
        ...
        if loan.lower() == "true":
            ...
            data["loan"][prod_name]["purchase"][bill_no]["payment"] = "Credit"
